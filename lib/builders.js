...
export function isPriority1(builder) {
    if (!isBuilder(builder)) return false;
    if (!isRelease(builder)) return false;
    if (isStable(builder)) return false;
    if (isPerf(builder)) return false;
    if (isNonUnified(builder)) return false;
    return isWPE(builder) || isGTK(builder);
}

export function isPriority2(builder) {
    if (!isTester(builder)) return false;
    if (!isRelease(builder)) return false;
    if (isStable(builder)) return false;
    if (isPerf(builder)) return false;
    if (isNonUnified(builder)) return false;
    if (isGTK4(builder) || isWayland(builder)) return false;
    if (isWebDriver(builder)) return false;
    return isWPE(builder) || isGTK(builder);
}

export function isPriority3(builder) {
    // Debug bots now combine building and running tests in a single bot tagged
    // "Tests" (the old *-Debug-Build bots are retired). Mirror isPriority2 but
    // match Debug instead of Release.
    if (!isTester(builder)) return false;
    if (!isDebug(builder)) return false;
    if (isStable(builder)) return false;
    if (isPerf(builder)) return false;
    if (isNonUnified(builder)) return false;
    if (isGTK4(builder) || isWayland(builder)) return false;
...
export function isPriority5(builder) {
    if (!isStable(builder)) return false;
    if (!isRelease(builder)) return false;
    return isWPE(builder) || isGTK(builder);
}

export function isPriorityOther(builder) {
    if (!(isWPE(builder) || isGTK(builder))) return false;
    return !(isPriority1(builder) || isPriority2(builder) || isPriority3(builder) || isPriority5(builder));
}

export function classifyByPriority(builders) {
    const priorities = { p1: [], p2: [], p3: [], p5: [], other: [], jsconly: [], retired: [] };
    for (const builder of builders) {
        if (isRetired(builder)) {
            if (isRelevantPlatform(builder)) priorities.retired.push(builder);
        } else if (isPriority1(builder)) priorities.p1.push(builder);
        else if (isPriority2(builder)) priorities.p2.push(builder);
        else if (isPriority3(builder)) priorities.p3.push(builder);
        else if (isPriority5(builder)) priorities.p5.push(builder);
        else if (isJSCOnlyLinux(builder)) priorities.jsconly.push(builder);
        else if (isPriorityOther(builder)) priorities.other.push(builder);
    }
    return priorities;
}
